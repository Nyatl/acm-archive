...
struct Node {
	...
	Node(ll l, ll r, ll q) : l(l), r(r), q(q) {}
};
...
ll rec(ll x) {
	if (x == 0) return -1;
	if (nodes.count(x)) return x;
	ll l = (x + 1) / 2;
	ll r = x / 2;
	nodes[x] = Node(rec(l), rec(r), x);
	return x;
}
...
void solve() {
	...
	printf("%d\n", sz(nodes) - 1);
	for (auto &z : nodes) {
		if (z.second.q == 0) continue;
		printf("%d %d\n", num(z.second.l), num(z.second.r));
	}
	assert(sz(nodes) - 1 <= 125);
	printf("%d\n", num(n));
	//dbg(nodes);
}
